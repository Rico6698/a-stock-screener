#!/usr/bin/env python3
"""
A股量化选股脚本 v6 —— 1000元本金尾盘短线筛选（稳健高胜率版）
相比 v5 的优化（目标：1000元股本、每天稳赚约0.5%）：
  - 价格区间放宽 3-5元 → 3-10元（1000元本金，1手≤1000元），扩大选股范围
  - 放宽 v5 过严的过滤（v5 在早盘9:26因换手率不足连续两天选0只）：
      换手率下限 1.0→0.8、上限 20→25；净利润同比 10→5；近5日涨幅 18→25；乖离率 5→6
  - 新增 20日平均振幅指标，低波动加分（求稳，匹配每天0.5%目标）
  - 适配尾盘14:50运行：全天数据完整，换手率/量能/突破均为真实信号
  - 保留 v5 的评分结构：趋势提权(多头排列15) + 风险扣分(超买/乖离/追高)
核心理念：尾盘确认全天站稳 + 顺势而为 + 低波动 + 不追高 + 不接盘
"""
...
class Config:
    def __init__(self):
        # --- 资金适配：1000元 → 1手 ≤ 1000元 → 股价 ≤ 10.0 ---
        self.min_price = 3.0
        self.max_price = 10.0
        self.min_amount = 30000000       # 最低成交额 3000万
        self.max_pe = 50                 # PE 上限
        self.max_debt_ratio = 60         # 资产负债率上限
        self.min_profit_growth = 5       # 净利润同比增速下限（放宽，求稳）
        self.min_turnover_rate = 0.8     # 最低换手率 %（放宽）
        self.max_turnover_rate = 25.0    # 换手率上限 %(异常活跃视为出货)
        self.max_5d_gain = 25.0          # 近5日涨幅上限 %(追高过滤，放宽)
        self.max_1d_drop = -5.0          # 近1日跌幅下限 %(接飞刀过滤)
        self.max_bias5 = 6.0             # 5日乖离率上限 %(超买扣分)
        self.max_amplitude = 5.0         # 20日平均振幅上限 %(低波动优先)
        self.min_days = 15
        self.lookback_days = 60
        self.max_candidates = 5
        self.threads = 8
        self.cache_ttl_hours = 2
        self.cache_dir = ".stock_cache_v6"
        self.output_format = "table"
        self.ci_mode = False

    @classmethod
    def from_cli(cls):
        p = argparse.ArgumentParser(description="A股短线选股 v6 (1000元本金尾盘稳健版)")
        ...
        p.add_argument("--min-growth", type=float, default=5)
        p.add_argument("--min-turnover", type=float, default=0.8)
        p.add_argument("--max-turnover", type=float, default=25.0)
        p.add_argument("--max-5d-gain", type=float, default=25.0)
        p.add_argument("--max-amplitude", type=float, default=5.0)
        ...
        cfg.max_turnover_rate = args.max_turnover
        cfg.max_5d_gain = args.max_5d_gain
        cfg.max_amplitude = args.max_amplitude
...
HISTORY_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".analysis_history_v6.json")
...
CN_COLUMNS = {
    ...
    "bb_pos": "布林位置", "bias5": "5日乖离率",
    "gain_1d": "近1日涨幅", "gain_5d": "近5日涨幅", "gain_20d": "近20日涨幅",
    "amplitude_20d": "20日振幅", "tech_reason": "技术理由",
}
...
def check_technical(code: str, code_raw: str, cfg: Config) -> Optional[dict]:
    try:
        ...
        # ===== 近期涨幅与乖离率 =====
        gain_1d = (close[-1] / close[-2] - 1) * 100 if n >= 2 and close[-2] > 0 else 0
        gain_5d = (close[-1] / close[-6] - 1) * 100 if n >= 6 and close[-6] > 0 else 0
        gain_20d = (close[-1] / close[-21] - 1) * 100 if n >= 21 and close[-21] > 0 else 0
        bias5 = (last_close - ma5) / ma5 * 100 if ma5 > 0 else 0

        # ===== v6 新增：20日平均振幅（低波动优先） =====
        amplitude_20d = 0.0
        if n >= 20:
            amps = [(high[i] - low[i]) / close[i] * 100 if close[i] > 0 else 0 for i in range(n - 20, n)]
            amplitude_20d = float(np.mean(amps))
...
def main():
    ...
    log.info("=" * 60)
    log.info("A股短线选股 v6（1000元本金 — 尾盘稳健高胜率版）")
    log.info(f"价格{cfg.min_price}-{cfg.max_price}元 PE≤{cfg.max_pe} 成交额≥{cfg.min_amount/10000:.0f}万")
    log.info(f"换手率{cfg.min_turnover_rate}-{cfg.max_turnover_rate}% 近5日涨幅≤{cfg.max_5d_gain}% 振幅≤{cfg.max_amplitude}%")
    log.info("=" * 60)
    ...
    # 换手率区间过滤（下限流动性，上限排除异常活跃出货）
    if "turnover_rate" in df.columns:
        df["turnover_rate"] = pd.to_numeric(df["turnover_rate"], errors="coerce")
        before = len(df)
        df = df[(df["turnover_rate"] >= cfg.min_turnover_rate) &
                (df["turnover_rate"] <= cfg.max_turnover_rate)].copy()
        log.info(f"④ 换手率{cfg.min_turnover_rate}-{cfg.max_turnover_rate}%: {before} -> {len(df)}")
    ...
    for _, row in df.iterrows():
        ...
        if tech is None:
            continue
        if tech["gain_5d"] > cfg.max_5d_gain:
            risk_filtered += 1
            continue  # 追高
        if tech["gain_1d"] < cfg.max_1d_drop:
            risk_filtered += 1
            continue  # 当日大跌(接飞刀)
        passed_rows.append((row, tech))
    log.info(f"⑩ 风险硬过滤(追高/大跌): 排除 {risk_filtered} 只, 剩余 {len(passed_rows)} 只")

    # ---- 评分（v6：趋势提权 + 风险扣分 + 低波动加分） ----
    results = []
    for row, tech in passed_rows:
        code = row["stock_code"]
        ...
        # ===== 动量与位置信号 =====
        if tech["kdj_gold"]:
            score += 8; reason.append("KDJ金叉")
        if tech["vol_expand"]:
            score += 8; reason.append("温和放量")
        if tech["vol_trend_up"]:
            score += 6; reason.append("量能递增")
        ...
        # ===== 位置合理（收紧区间） =====
        if 40 < tech["rsi_14"] < 65:
            score += 4; reason.append("RSI有空间")
        if 0.4 < tech["bb_pos"] < 0.75:
            score += 4; reason.append("布林中上轨")
        if tech["avg_amount_20d"] >= 50000000:
            score += 6; reason.append("流动性充足")

        # ===== v6 新增：低波动加分（求稳，匹配每天0.5%目标） =====
        if tech["amplitude_20d"] < 3:
            score += 5; reason.append("低波动稳健")
        elif tech["amplitude_20d"] > cfg.max_amplitude:
            score -= 4; reason.append("波动过大")

        # ===== 风险扣分（负权重，提高胜率关键） =====
        if tech["rsi_14"] >= 75:
            score -= 6; reason.append("RSI超买")
        if tech["bias5"] > cfg.max_bias5:
            score -= 8; reason.append("乖离过大")
        if tech["gain_5d"] > 15:
            score -= 8; reason.append("短期追高")
        if tech["gain_20d"] > 30:
            score -= 5; reason.append("累计涨幅过大")
        ...
        results.append({
            ...
            "bb_pos": tech["bb_pos"],
            "bias5": tech["bias5"],
            "gain_1d": tech["gain_1d"],
            "gain_5d": tech["gain_5d"],
            "gain_20d": tech["gain_20d"],
            "amplitude_20d": tech["amplitude_20d"],
    ...
    if cfg.output_format == "json":
        ...
        for i, (_, r) in enumerate(top.iterrows(), 1):
            ...
            print(f"   指标: RSI={r['rsi_14']:.1f}  乖离率{r['bias5']:.2f}%  振幅{r['amplitude_20d']:.2f}%  近5日{r['gain_5d']:+.2f}%")
