#!/usr/bin/env python3
"""
A股量化选股脚本 v5 —— 500元本金短线筛选（高胜率优化版）
相比 v4 的优化（目标：提高胜率）：
  - 新增风险硬过滤：近5日涨幅过大(追高)、当日大跌(接飞刀)、换手率异常(可能出货) 直接排除
  - 新增技术指标：5日乖离率 BIAS5、近1/5/20日涨幅
  - 评分重构：趋势信号提权(多头排列 12→15)、KDJ金叉降权(10→8)
  - 新增风险扣分项：RSI超买、乖离过大、短期累计涨幅过大、异常活跃
  - 位置区间收紧：RSI 40-65、布林 0.4-0.75
核心理念：顺势而为 + 不追高 + 不接盘 + 风险过滤
"""
...
class Config:
    def __init__(self):
        ...
        self.max_turnover_rate = 20.0    # 换手率上限 %(异常活跃视为出货)
        self.max_5d_gain = 18.0          # 近5日涨幅上限 %(追高过滤)
        self.max_1d_drop = -5.0          # 近1日跌幅下限 %(接飞刀过滤)
        self.max_bias5 = 5.0             # 5日乖离率上限 %(超买扣分)
        self.min_days = 15
        self.lookback_days = 60
        self.max_candidates = 5
        self.threads = 8
        self.cache_ttl_hours = 2
        self.cache_dir = ".stock_cache_v5"
        self.output_format = "table"
        self.ci_mode = False

    @classmethod
    def from_cli(cls):
        p = argparse.ArgumentParser(description="A股短线选股 v5 (500元本金高胜率优化版)")
        ...
        p.add_argument("--max-turnover", type=float, default=20.0)
        p.add_argument("--max-5d-gain", type=float, default=18.0)
...
HISTORY_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".analysis_history_v5.json")
...
def main():
    ...
    log.info("=" * 60)
    log.info("A股短线选股 v5（500元本金 — 高胜率优化版）")
    log.info(f"价格{cfg.min_price}-{cfg.max_price}元 PE≤{cfg.max_pe} 成交额≥{cfg.min_amount/10000:.0f}万")
    log.info(f"换手率{cfg.min_turnover_rate}-{cfg.max_turnover_rate}% 近5日涨幅≤{cfg.max_5d_gain}%")
    log.info("=" * 60)
    ...
    if cfg.output_format == "json":
        ...
        for i, (_, r) in enumerate(top.iterrows(), 1):
            ...
            print(f"   指标: MA5斜率{r['ma5_slope']:.2f}%  RSI={r['rsi_14']:.1f}  乖离率{r['bias5']:.2f}%  近5日{r['gain_5d']:+.2f}%")
