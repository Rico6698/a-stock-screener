#!/usr/bin/env python3
"""
A股量化选股脚本 —— 1000元本金短线筛选
侧重：寻找有上涨动能的股票（均线多头/量能配合/MACD金叉）
"""
...
class Config:
    def __init__(self):
        # --- 资金适配：1000元 → 1手 ≤ 1000元 → 股价 ≤ 9.9 ---
        self.min_price = 3.0
        self.max_price = 9.9
        self.min_amount = 30000000       # 最低成交额 3000万
        self.max_pe = 60                 # 放宽 PE
        self.max_debt_ratio = 65         # 放宽负债率
        self.min_profit_growth = 5       # 放宽净利润增速
        self.min_days = 15
        self.lookback_days = 60          # 更长回看期，捕捉趋势
        self.max_candidates = 5
        self.threads = 8
        self.cache_ttl_hours = 2
        self.cache_dir = ".stock_cache_1000"
        self.output_format = "table"
        self.ci_mode = False

    @classmethod
    def from_cli(cls):
        p = argparse.ArgumentParser(description="A股短线选股 (1000元本金)")
        p.add_argument("--min-price", type=float, default=3.0)
        p.add_argument("--max-price", type=float, default=9.9)
        p.add_argument("--min-amount", type=float, default=30000000)
        p.add_argument("--max-pe", type=float, default=60)
        p.add_argument("--max-debt", type=float, default=65)
...
HISTORY_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".analysis_history_1000.json")
...
def check_technical(code: str, code_raw: str, cfg: Config) -> Optional[dict]:
    try:
        ...
        if n >= 26:
            ...
            dea_prev = 0.2 * dif_prev + 0.8 * dif_prev  # 简化
            macd_bull = (dif - dea) > 0
            # 金叉: 前一日 DIF <= DEA，今日 DIF > DEA
            macd_gold = (dif_prev <= dea_prev) and (dif > dea)

        # 4. 温和放量（今日量能 > 20日均量 1.2-2.5倍，不是天量）
        ...
        breakout = False
        if n >= 20:
            recent_high = float(max(close[-20:-1]))  # 不含今天
            breakout = last_close > recent_high

        # 6. RSI(14) — 40-70 之间最佳（有上涨空间但不过热）
        rsi = 50.0
        if n >= 15:
        ...
        return {
            ...
            "last_close": last_close, "low_3d": float(min(low[-3:])),
        }
    except Exception:
        return None
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        if tech["macd_gold"]:
            score += 15; reason.append("MACD金叉")
        if tech["breakout"]:
            score += 15; reason.append("突破前高")

        # ===== 辅助信号 =====
        if tech["avg_amount_20d"] >= 50000000:
            score += 8;  reason.append("流动性充足")
        if tech["on_ma5"]:
            score += 8;  reason.append("站上5日线")
        if tech["on_ma10"]:
            score += 5;  reason.append("站上10日线")
        if tech["ma5_slope"] > 0:
            score += 8;  reason.append("MA5上行")
        if tech["ma10_slope"] > 0:
            score += 5;  reason.append("MA10上行")
        if tech["ma20_slope"] > -0.3:
            score += 3;  reason.append("MA20趋稳")
        if tech["vol_expand"]:
            score += 10; reason.append("温和放量")
        if tech["consecutive_up"]:
            score += 8;  reason.append("连续小阳")
        if tech["macd_bull"]:
            score += 5;  reason.append("MACD多头")
        if 40 < tech["rsi_14"] < 70:
            score += 5;  reason.append("RSI有空间")
        if 0.5 < tech["bb_pos"] < 0.85:
            score += 5;  reason.append("布林中上轨")
    ...
    if cfg.output_format == "json":
        ...
        print(f"\n{'名称':<10}{'代码':<8}{'价格':<8}{'1手金额':<8}{'市盈率':<8}{'评分':<6}{'多头':<6}{'金叉':<6}{'突破':<6}{'技术理由'}")
        print("-" * 140)
        for _, r in top.iterrows():
            print(f"{r['name']:<10}{r['code']:<8}{r['price']:<8.2f}{r['hand_cost']:<8.0f}"
                  f"{r['pe']:<8.1f}{r['score']:<6}{r['bull_align']:<6}{r['macd_gold']:<6}{r['breakout']:<6}{r['tech_reason']}")
        ...
        for i, (_, r) in enumerate(top.iterrows(), 1):
            ...
            print(f"   信号: 多头排列={r['bull_align']}  MACD金叉={r['macd_gold']}  突破前高={r['breakout']}  温和放量={r['vol_expand']}")
